...
import com.alirizakaygusuz.jwt_verification_otp_token_poc.authorization.relation.userrole.repository.RoleUserRepository;
...
public class MockUserSeederService {
    ...
    public void seed(){
        if (authUserRepository.count() == 0) {
            ...
            UserRole adminRelation =UserRole.builder()
            		.authUser(admin)
            		.role(adminRole)               
            		.build();
            
            UserRole userRelation=UserRole.builder()
            		.authUser(user)
            		.role(userRole)
            		.build();
